      ...
      >
        {relatedVideos.map((related) => (
          <SwiperSlide key={related.id}>
            <Link className="relative group h-full scale-100" href={"/watch"}>
              <Image
              ...
              <div className="absolute bottom-2 left-4">
                ...
                <div className="sm:flex block gap-2 items-center opacity-0 group-hover:opacity-100 transition duration-300">
                  <p className="text-white text-[14px]">{related.category}</p>
                  <FontAwesomeIcon
                    className="h-1 w-1 text-white sm:block hidden"
                    icon={faCircle}
                  />
                  <p className="text-white text-[14px]">{related.duration}</p>
                </div>
              </div>
            </Link>
          </SwiperSlide>
