...
export async function POST(request) {
  ...
  try {
    ...
    const result = await db.collection('episode').insertOne({ thumbnail, name, description, status });
...
export async function PUT(request) {
  ...
  try {
    ...
    const client = await clientPromise;
    const db = client.db('tichannel');
    const result = await db.collection('episode').updateOne({ _id: new ObjectId(id) }, { $set: updateData });
